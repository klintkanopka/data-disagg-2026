...
write_csv(nhpi_yield, 'tab/nhpi_yield.csv')

ggplot(nhpi_yield, aes(x=term_cd, y=yield_rate, color=group)) +
  geom_line() +
  geom_point() +
  scale_color_viridis_d() +
  labs(title='multiracial nhpi yield rates') +
  theme_minimal()

ggsave('fig/nhpi-yield.png', height=6, width=8)
